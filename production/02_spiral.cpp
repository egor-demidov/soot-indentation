...
int main() {
    ...
    // Parse the CL args
    size_t bond_to_break = 1;
    const std::string dump_dir = "run";
    const std::string data_file = "../plots/production/02_spiral.dat";
    ...
    for (size_t i = 0; i < num_edges*3; i ++) {
        ...
        Eigen::Matrix3d rot;
        rot = Eigen::AngleAxis(-angle * double(i), Eigen::Vector3d::UnitY());
        dx = rot * dx;
        x0.emplace_back(x0.back() + dx + dy);
    }
    ...
    std::tuple<size_t, size_t, size_t> anchor_plane = {0, 1, 2};
    ...
    v0[tip_point] = Eigen::Vector3d::UnitY() * 0.1;
