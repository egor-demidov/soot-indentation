...
static size_t boundary_particle_counter = 1;
...
void write_boundary_particles(std::string const & dir, std::vector<Eigen::Vector3d> const & x, double r_part,
    ...
    stringstream path;
    path << dir << "/boundary_particles_" << boundary_particle_counter << ".vtk";
    boundary_particle_counter ++;
    ...
    std::array<Eigen::Vector3d, 4> particles = {
            x[std::get<0>(anchored_particles)],
            x[std::get<1>(anchored_particles)],
            x[std::get<2>(anchored_particles)],
            x[tip_particle]
    };
...
void write_necks(std::string const & dir, std::vector<Eigen::Vector3d> const & x, double r_part,
    ...
    for (size_t i = 0; i < n_part - 1; i ++) {
        for (size_t j = i + 1; j < n_part; j ++) {
            if (bonded_contacts[i * n_part + j]) neck_indices.emplace_back(i, j);
        }
    }
